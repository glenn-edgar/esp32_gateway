
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <esp_types.h>
#include "watchdog.h"
#include "gpio.h"
#include "debounce_ctrl.h"
#include "mqtt_ctrl.h"
#include "chain_flow_assembler/cf_external_functions.h"

#define SWITCH_NUMBER 8
static int gpio_switch_pins[SWITCH_NUMBER] = {23,22,21,19,18,17,16};

static  DEBOUNCE_CTRL  debounce_ctrl[SWITCH_NUMBER];



bool debounce_control_update( DEBOUNCE_CTRL *debounce_ctrl, unsigned new_value);

int pat_watch_dog(unsigned link_id, unsigned param_1,
  unsigned param_2, unsigned param_3, unsigned event, unsigned data)
{
   wdt_reset_task_time(); 
   return CF_DISABLE;   
}


int sample_switches(unsigned link_id, unsigned param_1,
  unsigned param_2, unsigned param_3, unsigned event, unsigned data)
{
    unsigned temp;
    bool     transistion;
    
    for( int i=0;i<SWITCH_NUMBER;i++)
    {
        temp = gpio_read_value(gpio_switch_pins[i]);
        transistion = debounce_control_update( &debounce_ctrl[i], temp);
        if(transistion == true)
        {
            printf("transition %d %d %d \n",i,transistion,temp);
        }
        
    }
    return CF_DISABLE;    
}


int add_watch_dog(unsigned link_id, unsigned param_1,
  unsigned param_2, unsigned param_3, unsigned event, unsigned data)
{
    
   wdt_task_subscribe(); 
   return CF_DISABLE;     
}  


int setup_gpio(unsigned link_id, unsigned param_1,
  unsigned param_2, unsigned param_3, unsigned event, unsigned data)
{
    for( int i=0;i<SWITCH_NUMBER;i++)
    {
        gpio_config_input_pin( gpio_switch_pins[i] );
        gpio_config_pull_up( gpio_switch_pins[i] );
        debounce_control_int( &debounce_ctrl[i],10 );
        
    }
    return CF_DISABLE;    
}

int wait_for_mqtt_connect(unsigned link_id, unsigned param_1,
  unsigned param_2, unsigned param_3, unsigned event, unsigned data)

{
    MQTT_STATE  state;
    int return_value;
    state = mqtt_ctl_get_state();
    if( (state == RESTORE_SUBSCRIPTIONS) || ( state ==MQTT_OPERATIONAL_STATE ))
    {
    return_value = 1;
    }
    else
    {
        return_value = 0;
    }
    return return_value;
}    
