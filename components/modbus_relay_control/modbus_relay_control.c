...
static void modbus_relay_task( void *ptr)
{
   ...
   {       
        ...
        if( mbcontroller_get_param_info(&reg_info, 1000)!= ESP_OK){continue;}
        if (reg_info.type == MB_EVENT_COILS_WR) 
        {

            modbus_relay_update_io();
        }
        else if(reg_info.type == MB_EVENT_HOLDING_REG_WR)  
        {
...
#if 1
        else if(reg_info.type == MB_EVENT_HOLDING_REG_RD)  
        {
